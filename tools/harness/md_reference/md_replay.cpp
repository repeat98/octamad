...
//
//   md_replay <capture dir> [--reloc] [--driver] [--init] [--boot] [--interpreter] [out.wav slot]
//
...
// --interpreter uses the reference fork's instruction interpreter. Required
// for the transport oracle: the fork's multi-instruction JIT corrupts 48-bit
// X state across a DO loop (md_phase_probe.cpp). Capture audio generated by
// that JIT is therefore diagnostic only for this gate.
// MD_REPLAY_STATE_OUT=<file> dumps each post-render X/Y voice record.
//
...
int main(int _argc, char** _argv)
{
	...
	std::cout << "execution: " << (interpreter ? "interpreter" : "JIT") << "\n";
	...
	auto runTo = [&](std::initializer_list<TWord> _stops) -> TWord
	{
		for(uint64_t n = 0; n < 2'000'000; ++n)
		{
			...
			if(interpreter) dsp.execInterpreter();
			else dsp.exec();
#ifdef MD_REPLAY_READS
			g_curPc = g_noPc;
#endif
	...
	for(size_t i = 0; i < groups.size() && i < maxBlocks; ++i)
	{
		...
		else if(g.state)
		{
			...
			for(size_t k = 0; k < 16 && k < rec.words.size(); ++k)
				memory.set(MemArea_Y, base + static_cast<TWord>(k), rec.words[k]);
		runTo({donePc});
		// Optional post-render voice records for transport diagnostics.
		static std::ofstream stateDump = [] {
			const char* f = std::getenv("MD_REPLAY_STATE_OUT");
			return f ? std::ofstream(f) : std::ofstream();
		}();
		if(stateDump.is_open())
		{
			stateDump << i << " " << rec.slot;
			for(auto area : {MemArea_X, MemArea_Y})
				for(TWord k = 0; k < 64; ++k)
					stateDump << " " << std::hex << memory.get(area, base + k) << std::dec;
			stateDump << "\n";
		}
