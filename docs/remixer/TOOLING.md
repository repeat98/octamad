...
| `ot_spec` | `tools/hw/ot_spec.py` | one JSON spec over a project's parts and patterns: `apply` (FX ids by module name, every knob by name, part names, trigs, AMP/FX1/FX2 locks by knob name, `clear`), `report` (the same shape back), `diff` (two projects, field by field); checksum + read-back on `.work` and `.strd` |
