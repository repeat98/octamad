...
| 13 Sep 2026 | nordseele | [`octalab`](https://github.com/nordseele/octalab) at `40ffa53` (MIT, findings only, then named `octalab-notes`), from an Octatrack MKI running our loader: FS layer, slot loading, Parts, the card's files, step records and lock stores, the input layer, menus, the platform reserve on hardware | `STORAGE.md`; `PARAM_PAGES.md` §5g; `PANEL.md` §4b; `MAINMENU.md` §2, §5; `PLACEMENT.md`; `MIDI.md` (PLAYBACK `machine*6`); `tools/hw/ot_project.py` (trig masks `0x40`/`0x48`) |
| 22 Sep 2026 | nordseele | [`octalab`](https://github.com/nordseele/octalab) `40ffa53..e0dc56d` (repo renamed from `octalab-notes` in the interval): a FAT directory record's first cluster is the LONG at `+0x11e`, not the word at `+0x120` (a folder past cluster 65535 on FAT32 was not listed); a second, more general WAV writer `0x40024168` driven by RAM globals `0x460be9e8`/`0x460be9ec` shared with the MAIN MENU select widget's staging; one recorder-reserve measurement (16.0 s, 460 × 0x1800 B blocks) that does not match this repo's own 6,144-byte recorder pool; a grid-recording trap where a custom page's popup can be torn down without its closed callback, leaving a trig held for good; the SETUP window's drawing primitives and font records. Also downgraded two of their own 13 Sep claims to 🟡 (current pattern/part/machine-type mapping "not isolated on hardware"; the `0x10` trigless-lock type label "still tentative") — this repo's own readings of those facts (`RECORDER.md`, `PARAM_PAGES.md`) were already independent and unaffected | `STORAGE.md` §1; `SAMPLE_SAVE.md` §1b; `RECORDER.md` §2; `PANEL.md` §4c |
