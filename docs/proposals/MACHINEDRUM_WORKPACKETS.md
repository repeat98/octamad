...
| WP-A2 The layout | todo | | | | | redo for core 1 (MD on T1–T4, decided 24 Sep); the core-0 map `3573d25` is superseded; inputs: the core-1 ledger and WP-A7's placement plan |
| WP-A7 The X/Y flip audit | done | `machinedrum` | this commit | 2026-09-24 | `machinedrum_reports/WP-A7.md` | all tables and the 16 P-I buffers fit core 1's private X/Y at 178 flips + 14 rewrites (+7.8 instr/sample worst); only the sine and code overflow stay in the window; D6 accepts the cost |
...
| `md_forms` | The emulator decoder's instruction form for each word (built with the others) |
| `md_flip.py` | Which external data can leave the window: reads `md_reads.sh`'s per-instruction `access.txt` files (WP-A7) |
...
- **Open decisions for the user** (packets that need one say so):
  ...
  - D6, whether to take WP-A7's placement plan: everything but the sine in
    core 1's private X/Y, at 178 one-bit flips and 14 rewritten instructions
    (+7.8 instructions per sample worst case), the rewrites needing relocator
    support for inserted words. Without them the P-I buffers (24,576 words)
    stay in the window.
