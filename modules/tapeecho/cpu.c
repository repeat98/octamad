...
_Static_assert(sizeof(TapeState) == 200, "hooks.s allocation");
...
static __attribute__((noinline)) void record_settled(TapeState *restrict s,
    ...
    for (unsigned i=0;i<TE_FRAMES;++i) {
        ...
        int32_t left,right;
        if (mix==2147483640) {
            /* amplify() is a multiple of eight. For an unclipped result,
             * multiplication by (2^31-8)/2^31 loses exactly one Q23 LSB
             * when wet>dry, otherwise zero. Keep that old rounding quirk. */
            left=clip((wet>>3)-(wet>l));
            right=clip((wet>>3)-(wet>r));
        } else {
