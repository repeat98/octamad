...
def main():
    ...
    has_dram = any(unit.dram for key in registry.remix(remix).modules
                   for unit in mods[key].linked)
    if has_dram:
        # Unicorn does not execute the platform's compressed DRAM loader.
        # Boot that loader under the real ColdFire port, then use its loaded
        # bytes for this isolated UI test. Never seed a different build's
        # runtime from a stale out/platform directory.
        if image.read_bytes() != (ROOT / 'out/mainos_bus.bin').read_bytes():
            sys.exit('verify_repitch_ui: DRAM image must match the current build/layout')
        layout = json.loads((ROOT / 'out/platform/layout.json').read_text())
        base, end = layout['base'], layout['runtime_end']
        with tempfile.TemporaryDirectory(prefix='repitch-ui-') as td:
            dump = pathlib.Path(td) / 'runtime.bin'
            r = subprocess.run([str(ROOT / 'out/emu/ot_emu'), '--image', str(image),
                                '--max', '80000000', '--mem-dump',
                                f'0x{base:x},{end-base}={dump}'],
                               capture_output=True, text=True, cwd=ROOT)
            if r.returncode or 'HANDOFF' not in r.stdout or not dump.exists():
                sys.exit('verify_repitch_ui: ColdFire DRAM boot failed\n' + r.stdout[-1200:])
            runtime = dump.read_bytes()
        if len(runtime) != end - base:
            sys.exit('verify_repitch_ui: incomplete DRAM dump')
        boot = emu.boot(str(ROOT / 'out/raw/section_3_MAIN_OS.bin'))
        boot.uc.mem_write(emu.BASE, image.read_bytes())
        boot.uc.mem_write(base, runtime)
        print('  [ok] UI harness uses the combined image and its boot-loaded DRAM runtime')
    else:
