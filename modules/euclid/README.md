...
one of four destinations: 12 dB/octave low-pass, band-pass, high-pass, or
amplitude. The rhythm keeps rolling across pattern loops; PLAY resets phase.
...
FREQ is the base cutoff for LP/BP/HP and the base gain for AMP. DEPTH is
bipolar: its center leaves the destination static, positive values increase
it on a pulse, and negative values decrease it. RES controls the three
filter types and is inert for AMP. MIX zero is exact dry passthrough.

TYPE offers LP, BP, HP and AMP. The filter modes use the same two-pole TPT
state-variable core. AMP applies the Euclidean control signal as gain, so it
can produce rhythmic level envelopes, gates and stepped/random amplitude.
...
once per track even when Euclid occupies both FX slots.

`filter.asm` contains the stereo two-pole TPT state-variable core and AMP
tap. Cutoff ramps per sample. Its reciprocal denominator follows a moving
...
payloads, LP/BP/HP response, AMP unity/silence, cutoff sweeps, stock-row
